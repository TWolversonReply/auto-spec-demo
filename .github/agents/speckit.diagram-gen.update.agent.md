...
3. **Update the constitution** at `.specify/memory/constitution.md`:
   ...
   - Treat `images/context-map.png` as a CI-generated documentation artifact. Do NOT assume this file exists locally in the repository after running the command.
   - The command's responsibility is to keep source-of-truth inputs correct (`.specify/memory/constitution.md` and `model/context-map.cml`) so CI can generate/publish the image.
